...
class KdaBackwardStableTest(unittest.TestCase):
    def test_routing_preserves_pr_contract(self):
        select = _load_module("kda_backward_policy_test", OPS_DIR / "_kda_policy.py")._select_kda_bwd_optimized
    ...
    def test_metadata_compacts_empty_sequences_and_checks_indices(self):
        metadata = _load_module("kda_backward_policy_test", OPS_DIR / "_kda_policy.py")._canonical_kda_bwd_metadata
    ...
    def test_v2_header_matches_both_launchers(self):
        ...
        headers = tool.parse_headers(
            REPO_ROOT / "fla/ops/ascendc/kda/chunk_kda_bwd/op_host/op_api", {symbol})
        adapters = tool.adapter_calls(SRC_DIR)
        table = tool.parse_ctypes_table(OPS_DIR / "_aclnn_ctypes.py")
    ...
    def test_stable_dispatch_argument_order_and_stream(self):
        ...
        launch = mock.Mock(return_value=(1, 2, 3, 4, 5, None, 7, None))
        namespace.update(_op=mock.Mock(return_value=launch),
                         _host_ints=lambda x: ("host", x),
                         _current_stream_ptr=mock.Mock(side_effect=[17, 23, 31]))
        with mock.patch.dict(sys.modules, {"kda_test._kda_policy": policy}):
