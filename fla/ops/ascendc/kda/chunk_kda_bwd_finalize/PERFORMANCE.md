...
The description below is the historical pre-NaN-repair layout and performance
record. The selected saved-chain candidate retains two heads but uses stable
32-row bands and per-band final output stores; q/k remain intact between bands.
Current UB/workspace lifetimes and the three-shape qualification are documented
in [optimized_design.md](../chunk_kda_bwd/docs/optimized_design.md) and
[optimized_validation.md](../chunk_kda_bwd/docs/optimized_validation.md).
