...
if __name__ == "__main__":
    ...
    h_npu = (h_nt.squeeze(0) if cu_seqlens is not None else h_nt).contiguous().to(dtype).npu()
    dv_npu = dv.to(dtype).npu()
    do_npu = do.to(dtype).npu()
    dh_nt = dh.transpose(1, 2)
    dh_npu = (dh_nt.squeeze(0) if cu_seqlens is not None else dh_nt).contiguous().to(dtype).npu()
