...
| 名称 | 必选性 | Shape/Dtype | 说明 |
| --- | --- | --- | --- |
| `oOut` | 必选 | `[B,Hv,T,V]`（BNSD）；与 q 同 dtype | 前向输出固定使用 head-first 布局 |
...
| 名称 | 当前支持范围 | 说明 |
| --- | --- | --- |
| `layout` | A5 `useExp2=true` 支持 `BNSD/BSND/NTD/TND`；其他路径保持原有支持范围 | q/k/v 的输入布局；BSND/TND 输入在拼接路径内转为 head-first，o 固定输出 BNSD |
